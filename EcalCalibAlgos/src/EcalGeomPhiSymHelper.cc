...
void EcalGeomPhiSymHelper::setup(bool verbose, int statusThreshold,
                                 ...
                                 const EcalChannelStatus* chstatus)
{
    ...
    const std::vector<DetId>& barrelCells = geometry_->getValidDetIds(DetId::Ecal, EcalBarrel);
    ...
    const CaloSubdetectorGeometry *endcapGeometry = 
        geometry_->getSubdetectorGeometry(DetId::Ecal, EcalEndcap);
    ...
    const std::vector<DetId>& endcapCells = geometry_->getValidDetIds(DetId::Ecal, EcalEndcap);
    ...
    for(endcapIt=endcapCells.begin(); endcapIt!=endcapCells.end(); endcapIt++)
    {
        ...
        int chs= (*chStatus_)[*endcapIt].getStatusCode() & 0x001F;
        if( chs <=  statusThreshold_)
